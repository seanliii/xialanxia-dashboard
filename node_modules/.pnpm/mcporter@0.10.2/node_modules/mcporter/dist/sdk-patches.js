...
export function getStdioLogMode() {
    return stdioLogMode;
}
export function setStdioLogMode(mode) {
...
export function evaluateStdioLogPolicy(mode, hasStderr, exitCode) {
    if (!hasStderr) {
        return false;
    }
    if (mode === 'silent') {
...
function patchStdioClose() {
    ...
    StdioClientTransport.prototype.close = async function patchedClose() {
        ...
        const meta = (child ? PROCESS_BUFFERS.get(child) : undefined) ?? TRANSPORT_BUFFERS.get(transport);
        if (stderrStream) {
...
//# sourceMappingURL=sdk-patches.js.map