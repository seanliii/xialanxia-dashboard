...
function handleJsonArgsFlag(context) {
    return consumeJsonArgsFlag(context, '--json', '--json requires a JSON object value.');
}
function consumeJsonArgsFlag(context, flagName, missingValueMessage) {
    ...
    try {
        ...
        throw new Error(`Unable to parse ${flagName}: ${error.message}`, { cause: error });
    }
...
function handleNamedArgumentFlag(context) {
...
function normalizeLongFlagArgumentKey(rawKey) {
    if (!rawKey || rawKey.startsWith('-')) {
        return '';
    }
    return rawKey.replace(/-([a-zA-Z0-9])/g, (_match, char) => char.toUpperCase());
}
