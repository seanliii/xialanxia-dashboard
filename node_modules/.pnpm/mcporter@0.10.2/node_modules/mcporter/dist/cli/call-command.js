...
async function prepareCallRequest(runtime, args) {
    ...
    const schemaAwareArgs = await enforceSchemaAwareArgumentTypes(runtime, server, tool, hydratedArgs, parsed.schemaStringCoercionCandidates, parsed.schemaArrayCoercionCandidates, timeoutMs);
...
async function invokePreparedCall(runtime, prepared) {
    let invocation;
    try {
        invocation = await invokeWithAutoCorrection(runtime, prepared.server, prepared.tool, prepared.hydratedArgs, prepared.timeoutMs, prepared.parsed.output);
    }
    catch (error) {
...
function isErrorCallResult(result) {
...
async function enforceSchemaAwareArgumentTypes(runtime, server, tool, args, stringCandidates, arrayCandidates, timeoutMs) {
    if ((!stringCandidates || Object.keys(stringCandidates).length === 0) &&
...
function schemaAllowsArray(descriptor) {
    if (!descriptor || typeof descriptor !== 'object') {
        return false;
    }
    const record = descriptor;
...
async function invokeWithAutoCorrection(runtime, server, tool, args, timeoutMs, outputFormat) {
...
async function attemptCall(runtime, server, tool, args, timeoutMs, outputFormat, allowCorrection) {
    try {
...
async function maybeRetryResolvedTool(runtime, server, tool, args, timeoutMs, outputFormat, resolution) {
...
function extractMissingToolFromError(error) {
    ...
    const match = message.match(/Tool\s+([A-Za-z0-9._-]+)\s+not found/i) ?? message.match(/Unknown tool:?\s+([A-Za-z0-9._-]+)/i);
    return match?.[1];
}
function extractErrorMessageText(value) {
