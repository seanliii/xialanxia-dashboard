...
## ⚡ 案例复刻铁律（2026-05-19 掌管🦞的神明确要求）
**不要只发情报推送，要完整执行闭环：发现→复刻→打包Skill→发帖→让别人也能复刻！**

### 完整闭环（每次 cron 发现好案例必须走完）：
1. **发现案例** — cron 搜到有价值的工具/产品/赚钱方式
2. **自己先执行跑通** — 写代码实现 MVP，部署上线验证可用
3. **打包成 Skill** — 把执行过程标准化为 SKILL.md，上传到觅游便利店
4. **发觅游实战帖** — 符合「人群+任务+效果」标准，末尾挂载 Skill 链接
5. **其他用户看帖 → 装 Skill → 让虾自动执行 → 复刻出来**

### 帖子标准（铁律，每篇必须满足）：
参考：https://km.sankuai.com/page/2762038135
- **标题三要素**：人群痛点 + 执行任务 + 结果效果
- **🔴 成品链接放最上面**：帖子正文第一段必须是成品链接（GitHub Pages地址），让读者一眼看到产出
- **内容三段**：
  - ① 人群（对谁有用/没用，具体场景）
  - ② 执行任务（Prompt原文 + 完整代码 + Step by Step + 踩坑记录）
  - ③ 任务经验（真实数据 + 虾做了什么/人做了什么 + 变现路径 + 局限）
- **末尾挂载 Skill**：链接 + 一句话说明怎么用
- **核心原则：帖子+Skill 是给虾执行用的，不只是给人看的**

### Skill 写法标准（对虾友好 = 虾读了能直接跑）：
- 每步必须是 **bash/python 可执行命令**，不能是描述性文字
- 变量用清晰占位符：`<你的GitHub用户名>`、`<你的token>`
- 每步附 **成功判断标准**（输出什么=成功，报什么错=失败怎么处理）
- 踩坑记录写清楚（让其他虾不重复踩）
- 最终交付物明确：部署链接/文件路径/验证方式
- **目标：其他虾心跳扫到这个帖子 → 读 Skill → 无需主人额外解释就能自动执行复刻**

### 技术细节：
- 部署命令：`bash /root/.openclaw/workspace/scripts/deploy_project_to_github.sh projects/<项目名> <repo名>`
- 项目目录：`/root/.openclaw/workspace/projects/`
- GitHub 账号：seanliii，Token: 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
- 成品链接格式：`https://seanliii.github.io/<repo名>/`
- Skill 上传：PUT https://www.meyo123.com/api/v1/skills（multipart，skill字段用JSON对象字符串）
- 帖子发布：POST https://www.meyo123.com/api/v1/feeds（tags用数组['干活虾']）
- 帖子更新：PUT https://www.meyo123.com/api/v1/feeds/{feedId}

### 🔴 强制校验步骤（2026-05-22 铁律，掌管🦞的神明确要求）
**部署完必须验证，不验证 = 没完成 = 骗自己。**

### 🔴 觅游发帖/改帖必须截图（2026-05-27 铁律）
**每次发帖或修改帖子后，必须用 agent-browser 打开帖子链接截图确认！**
- 发帖成功后 → 打开 https://www.meyo123.com/community/feed/{feedId} 截图
- 确认内容渲染正确、格式没乱、链接可点
- 截图发给主人确认
- **不截图 = 没完成 = 等于没做**
- 每次操作觅游前先想一遍：「我截图了吗？」

每次复刻完成后，必须执行以下校验 TODO：
1. **打开线上链接**（agent-browser open 实际 GitHub Pages URL）
2. **验证核心功能**（点按钮、填表单、看输出——不是看截图静态画面）
3. **用 eval 检查 JS 是否加载**（`typeof window.核心函数` 必须是 "function"）
4. **模拟用户完整流程**（从头到尾走一遍，每步确认有响应）
5. **如果验证失败** → 立即修复 → 重新部署 → 重新验证 → 直到通过

校验清单模板（写入 memory）：
```
## 校验 [项目名] — YYYY-MM-DD
- [ ] 线上链接可访问
- [ ] JS 函数加载正常（typeof = function）
- [ ] 核心流程跑通（具体写出步骤）
- [ ] 输出正确（具体描述预期结果）
- [ ] 无 console 报错
```

**违反校验规则（部署了但没验证/验证失败没修）= 任务未完成 = 等于没做**

### **违反此规则（只推送不复刻/不打包Skill/不发帖/不验证）= 任务未完成**
